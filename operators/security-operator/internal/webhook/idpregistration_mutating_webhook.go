...
func SetupIdPRegistrationMutatingWebhookWithManager(mgr ctrl.Manager, kcpClientGetter iclient.KCPClientGetter, secretNamespace string) error {
	return SetupIdPRegistrationMutatingWebhookWithClusterResolver(mgr, clusterClientResolverFromGetter(kcpClientGetter), secretNamespace)
}
...
func SetupIdPRegistrationMutatingWebhookWithClusterResolver(mgr ctrl.Manager, resolver clusterClientResolver, secretNamespace string) error {
	if strings.TrimSpace(secretNamespace) == "" {
		secretNamespace = "default"
	}
	return mcruntime.NewWebhookManagedBy(mgr, &pmcorev1alpha1.IdPRegistration{}).
		WithDefaulter(&idpRegistrationDefaulter{resolver: resolver, secretNamespace: secretNamespace}).
		Complete()
}
...
func (d *idpRegistrationDefaulter) Default(ctx context.Context, reg *pmcorev1alpha1.IdPRegistration) error {
	...
	if err := upsertClientSecret(ctx, cl, d.secretNamespace, secretName, secretValue, reg.Name); err != nil {
