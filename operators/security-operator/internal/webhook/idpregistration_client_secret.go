...
func upsertClientSecret(
	...
	owner, labeled := existing.Labels[idpRegistrationSecretLabel]
	if labeled {
		if owner != registrationName {
			return fmt.Errorf("secret %q is owned by IdPRegistration %q", secretName, owner)
		}
	} else {
		return fmt.Errorf("secret %q already exists and is not managed by IdPRegistration", secretName)
	}
