...
func deleteManagedClientSecret(ctx context.Context, cl ctrlruntimeclient.Client, secretName, registrationName, namespace string) error {
	if strings.TrimSpace(secretName) == "" {
		return nil
	}

	secret := &corev1.Secret{}
	if err := cl.Get(ctx, ctrlruntimeclient.ObjectKey{Name: secretName, Namespace: namespace}, secret); err != nil {
