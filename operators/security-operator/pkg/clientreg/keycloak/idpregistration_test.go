...
func TestToKeycloakIdentityProviderFromRegistration_Discovery(t *testing.T) {
	...
	assert.Equal(t, PlatformMeshManagedByIdPRegistration, rep.Config[PlatformMeshManagedByKey])
...
func TestBrokerRedirectURI(t *testing.T) {
	assert.Equal(
		...
		BrokerRedirectURI("portal.localhost:8443", "/keycloak", "kaufmann", "dex"),
	)
	assert.Equal(
		t,
		"https://portal.localhost:8443/keycloak/realms/kaufmann/broker/test/endpoint",
		BrokerRedirectURI("portal.localhost:8443/", "/keycloak", "kaufmann", "test"),
	)
	assert.Empty(t, BrokerRedirectURI("", "/keycloak", "kaufmann", "dex"))
	assert.Empty(t, BrokerRedirectURI("portal.localhost:8443", "/keycloak", "", "dex"))
	assert.Empty(t, BrokerRedirectURI("portal.localhost:8443", "/keycloak", "kaufmann", "  "))
}
